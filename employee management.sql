...
-- Situation -1 
-- We are planning a cybersecurity project. Find all employees working in the IT department (DepartmentID: 3).
...
SELECT DEPARTMENTID , COUNT(*) AS COUNT_OF_EMPLOYEES FROM EMPLOYEES
GROUP BY  DEPARTMENTID;
...
SELECT DEPARTMENTID , MIN(DATEOFJOINING) AS EARLY_DATE FROM EMPLOYEES
GROUP BY  DEPARTMENTID;
...
SELECT DEPARTMENTID ,DATEOFJOINING, COUNT(*) AS COUNT_OF_EMPLOYEES FROM EMPLOYEES
WHERE YEAR(DATEOFJOINING) = "2024" 
GROUP BY  DEPARTMENTID, DATEOFJOINING;

-- Situation -9: 
-- For training allocation, count the number of employees with salaries between 
-- ₹50,000 and ₹70,000 in each department. 

SELECT DEPARTMENTID , SALARY, COUNT(*) AS COUNT_OF_EMPLOYEES FROM EMPLOYEES
WHERE SALARY BETWEEN 50000 AND 70000
...
SELECT LOCATION , COUNT(DEPARTMENTNAME) AS COUNT_OF_DEPARTMENTS FROM DEPARTMENTS
GROUP BY LOCATION;
...
SELECT DEPARTMENTNAME , SUM(ANNUALBUDGET) AS TOTAL_BUDGET FROM DEPARTMENTS
...
SELECT HEADOFDEPARTMENT , COUNT(DEPARTMENTNAME) AS COUNT_OF_DEPARTMENT FROM DEPARTMENTS
...
SELECT LOCATION , AVG(ANNUALBUDGET) AS AVG_BUDGET FROM DEPARTMENTS
GROUP BY LOCATION;
...
SELECT DEPARTMENTNAME, CONCAT_WS(",",DEPARTMENTNAME,LOCATION) AS ADDRESS FROM DEPARTMENTS;

-- Situation -3: 
-- To prepare a personalized email greeting, concatenate "Hello " with the employee's 
-- name and a comma.

SELECT CONCAT_WS(",","Hello",FIRSTNAME) AS GREETING_NAME FROM EMPLOYEES;

-- Situation -4: 
-- For creating a unique employee ID, concatenate the department ID and employee 
-- number. 

SELECT FIRSTNAME, LASTNAME, CONCAT_WS("-",EMPLOYEEID,DEPARTMENTID) AS UNIQUE_ID FROM EMPLOYEES ;

-- Situation -5: 
-- For creating a company contact list, concatenate the employee's phone number 
-- with their extension (if applicable).

SELECT DEPARTMENTNAME , CONCAT_WS("-","256",PHONENUMBER);

-- Situation -6: 
-- To display the complete address, concatenate the street, city, and postal code of 
-- each employee.

SELECT FIRSTNAME , CONCAT_WS("," , STREET, CITY, POATALCODE) FROM EMPLOYEES;

-- Situation -7: 
-- To create an employee login ID, concatenate the employee’s first name, last name, 
-- and their department ID.

SELECT EMPLOYEEID , CONCAT_WS("-",FIRSTNAME,LASTNAME,DEPARTMENTID) AS LOGINID FROM EMPLOYEES;

-- Situation -8: 
-- For sending out a personalized notification, concatenate the employee's department 
-- with their position. 

SELECT CONCAT_WS("-",DEPARTMENTNAME,POSITION) AS NOTIFICATION FROM DEPARTMENTS;

-- Situation -9: 
-- To display the employee's work status, concatenate their job title and current 
-- project. 

SELECT FIRSTNAME, CONCAT_WS("-", TITLE, PROJECT) FROM EMPLOYEES;

-- Situation -10: 
-- For an annual report, concatenate the year and employee’s first name to create a 
-- unique report reference code. 

SELECT EMPLOYEEID, CONCAT_WS("-",YEAR(DATEOFJOINING) , FIRSTNAME)AS REFERENCE_CODE FROM EMPLOYEES;
...
CHECK (EMPLOYEECATEGORY IN ("FullTime","PartTime","Contract"));
